...
# gen_path = './model/gen_spring_' + args.network + '_' + str(args.nodes) + '_id' + str(args.exp_id) + '.pkl'
#for example
dyn_path = './model/dyn_spring_BA_10_id1.pkl'
gen_path = './model/gen_spring_BA_10_id1.pkl'
#load model
...
def train():
    ...
    for idx, data in enumerate(test_loader):
        ...
        outputs = torch.zeros(x.size(0), args.control_steps+1,len(node_list), args.dim).to(device)
        ...
        for time in range(args.control_steps):
            ...
            for idx,j in enumerate(node_list):
            ...
            while len(controlled_node_list_copy)>1:
                ...
                for no in controlled_node_list_copy:
                    count = count +1
                    #loss = loss + torch.mean(torch.abs(temp_x[:, cur_con_node, 2:]-temp_x[:,no,2:]))
...
main_path = './model/'
control_path = main_path + 'control_spring_'+str(args.network)+'_ourmodel_ournet_model_id'+str(args.exp_id)+'.pkl'

data_path = './data/control_spring_'+str(args.network)+'_ourmodel_ournet_data_id'+str(args.exp_id)+'.pickle'
